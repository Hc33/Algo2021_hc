...
def main():
    """
    将perform_encoding.py得到的预测结果处理成.pkl文件
    打包压缩成zip
    """
    ...
    track = args['track']
    result_dir = args['result_dir']  # where has *_test.npy file
    if track == 'full_track':
        ROIs = ['WB']
    else:
    ...
    num_subs = 10
    subs=[]
    for s in range(num_subs):  # 全部10个受试者, sub01 -> sub10
        subs.append('sub'+str(s+1).zfill(2))

    results = {} 
    for ROI in ROIs: # [WB] or [LOC, FFA, ....]
        ROI_results = {}  # dict()
        for sub in subs:  # sub01 -> sub10
            ...
            if not os.path.exists(ROI_result_file):
                ...
                return
            ROI_result = np.load(ROI_result_file)  # load *_test.npy
